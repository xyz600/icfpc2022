...
fn solve_inner(image: &Image, row_list: &Vec<usize>, column_list: &Vec<usize>) -> State {
    ...
    eprintln!("final cost = {}", dp[0][0][row_list.len() - 1][column_list.len() - 1]);
...
pub fn solve(image: &Image) -> State {
    ...
    solve_inner(image, &row_list, &column_list)
}
